...
public class ServerDataEngine implements InterfaceDataNetwork {
	...
	public void askJoinTable(User user, GameTable table, boolean isPlayer) {
		...
		if (tableFull.connect(user, isPlayer)) {
			...
			// TOREVIEW est ce que c'est une bonne id�e d'afficher l'erreur de cette mani�re au client ? Le fiat est que
			// si on le fait pas, il attendra de mani�re infinie d'apres nos diag de sequence.
		...
		// }
		// else if(isLaunched)
		// this.comServer.raiseException(user.getPublicData().getUuid(), "Impossible de rejoindre une partie d�j�
		// commenc�e.");
		// else if(isFull)
		// this.comServer.raiseException(user.getPublicData().getUuid(), "Impossible de rejoindre une partie pleine.");
		// else
		// {
		// this.comServer.raiseException(user.getPublicData().getUuid(), "Erreur inconnue lors de la connexion � la
		// table");
		// // throw new Exception("Erreur inconnue lors de la connexion � la table");
		// }
		// }
		...
		// this.comServer.raiseException(user.getPublicData().getUuid(), "Impossible de regarder cette partie. Non
		// Autoris� par le Cr�ateur.");
		// else
		// {
		// this.comServer.raiseException(user.getPublicData().getUuid(), "Erreur inconnue lors de la connexion � la
		// table");
	...
	public void launchGame(User user) {
		...
		// throw new Exception("L'utilisateur n'a rejoint aucune table. Il faut �tre assit � une table pour lancer une
		// partie.")
		// else if(!userFull.getActualTable().getCreator().isSame(user))
		// throw new Exception("L'utilisateur n'est pas le createur de sa partie. Il faut �tre le createur pour lancer
		// une partie.");
